...
from middlewares.error_handler import AuthorizationError
...
import repositories.pedido_repository as ped_repository
...
def register_routes(app):
    ...
    app.add_url_rule("/health", "health_check", _health_check, methods=["GET"])
    app.add_url_rule("/admin/reset-db", "reset_database", _reset_database, methods=["POST"])
...
def _health_check():
    return jsonify({
        ...
        "database": "connected",
        "counts": {
            "produtos": produto_repository.count(),
            "usuarios": usuario_repository.count(),
            "pedidos": ped_repository.count(),
        },
        "versao": "1.0.0",
    }), 200
...
def _reset_database():
    if settings.SECRET_KEY == "change-me-in-production":
        raise AuthorizationError("SECRET_KEY não configurado — reset-db desabilitado")
